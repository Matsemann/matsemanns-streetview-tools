...
def save_video_frames(video_file: Path, output_folder: Path, frames: list[int], quality: int = 2,
                      progressbar: bool = True, cleanup: bool = True) -> None:
    ...
    if progressbar:
        with tqdm(total=len(frames), desc="Extract frames from video", leave=True) as pbar:
...
def _create_ffmpeg_image_content(images: list[Path]) -> str:
    lines = [
        f"file '{p.resolve()}'" for p in images
    ]
    return "\n".join(lines)


def join_images_to_video(images: list[Path], output_file: Path, metadata_create_time: datetime,
                         framerate: int = 1,
                         crf_quality: int = 23,
                         preset: str = "medium",
...
def inject_spatial_data(input_file: Path, output_file: Path):
    ...
    from spatialmedia.metadata_utils import inject_metadata, Metadata, generate_spherical_xml
    metadata = Metadata()
