...
from matsemanns_streetview_tools.metadata import get_exiftool_metadata_for_images_in_folder
...
@click.command()
@click.argument("image_folder", type=click.Path(exists=True, dir_okay=True, file_okay=False))
...
def video_from_folder(image_folder: str, out_folder: str) -> None:
    ...
    video_gpx = gpx.adjust_time(gpx_track, start_time=gpx_track.utc_time, delta=timedelta(seconds=1))
    ...
    join_images_to_video(image_paths, tmp_video, metadata_create_time=video_gpx.utc_time, framerate=1)
