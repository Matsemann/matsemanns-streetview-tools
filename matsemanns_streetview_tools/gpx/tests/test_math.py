...
from matsemanns_streetview_tools.gpx._math import (
    relative_distance,
...
def test_intersect_interpolate():
    ...
    t1, t2 = intersect_line_with_circle(p1, p2=p2, radius=Decimal("5"))
...
def test_get_angle_degrees():
    ...
    assert get_angle_degrees(Decimal(0), Decimal(1)) == approx(
        Decimal(0), allowed_error
    )  # up
    assert get_angle_degrees(Decimal(0), Decimal(-1)) == approx(
        Decimal(180), allowed_error
    )  # down
    assert get_angle_degrees(Decimal(1), Decimal(0)) == approx(
        Decimal(90), allowed_error
    )  # right
    assert get_angle_degrees(Decimal(-1), Decimal(0)) == approx(
        Decimal(270), allowed_error
    )  # left
    assert get_angle_degrees(Decimal(2), Decimal(6)) == approx(
        Decimal("18.4349489"), allowed_error
    )  # left up /
